...
class Player(GamePlayer):
    def __init__(self):
        self.name = "TSP Performance Version"
        self.group = '1A'
        self.myBot = None
    ...
    def permute(self, game_map, cur_pos, obj_list, obj_path, costMax):
        result = []
        ...
        for i in range( len(obj_list) ):
            objT = obj_list.pop(0)
            obj_path.append(objT)
            cost = self.cost( game_map, cur_pos, obj_path, costMax )
            if ( cost < costMax ):
                costMax = cost
            result.extend( obj_path )
            perms = self.permute(game_map, cur_pos, obj_list, obj_path, costMax)
            #for perm in perms:
            #    perm.insert(0, objT)
            #result.extend(perms)
            obj_path.pop()

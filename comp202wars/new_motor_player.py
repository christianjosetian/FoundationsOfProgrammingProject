...
class Player(GamePlayer):
    ...
    def value( self, game_map, cur_pos, i, j, object ):
        #get game dimensions, support rectangular
        MapLength = len(game_map)
        assert( MapLength == len(game_map[0]) )
        value = self.valueI( game_map, cur_pos, object )
        
        return value
    
    
    def valueI( self, game_map, cur_pos, object ):
    ...
    def step(self, game_map, turn, cur_pos):
        ...
        if cur_pos[0] > 0:
            if game_map[cur_pos[0] - 1][cur_pos[1]]:
                if GameObject == type(game_map[cur_pos[0] - 1][cur_pos[1]][0]):
                    fineUp = self.value(game_map, cur_pos, cur_pos[0] - 1, cur_pos[1], game_map[cur_pos[0] - 1][cur_pos[1]][0])
                           
        #fine motor control down
        if cur_pos[0] < MapLength - 1:
            if game_map[cur_pos[0] + 1][cur_pos[1]]:
                if GameObject == type(game_map[cur_pos[0] + 1][cur_pos[1]][0]):
                    fineDown = self.value(game_map, cur_pos, cur_pos[0] + 1, cur_pos[1], game_map[cur_pos[0] + 1][cur_pos[1]][0])
           
        #fine motor control left
        if cur_pos[1] > 0:
            if game_map[cur_pos[0]][cur_pos[1] - 1]:
                if GameObject == type(game_map[cur_pos[0]][cur_pos[1] - 1][0]):
                    fineLeft = self.value(game_map, cur_pos, cur_pos[0], cur_pos[1] - 1, game_map[cur_pos[0]][cur_pos[1] - 1][0])
                           
        #fine motor control right
        if cur_pos[1] < MapLength - 1:
            if game_map[cur_pos[0]][cur_pos[1] + 1]:
                if GameObject == type(game_map[cur_pos[0]][cur_pos[1] + 1][0]):
                    fineRight = self.value(game_map, cur_pos, cur_pos[0], cur_pos[1] + 1, game_map[cur_pos[0]][cur_pos[1] + 1][0])
        ...
        if cur_pos[0] > 0 and cur_pos[1] > 0:
            if game_map[cur_pos[0] - 1][cur_pos[1] - 1]:
                if GameObject == type(game_map[cur_pos[0] - 1][cur_pos[1] - 1][0]):
                    fineUpperLeft = self.value(game_map, cur_pos, cur_pos[0] - 1, cur_pos[1] - 1, game_map[cur_pos[0] - 1][cur_pos[1] - 1][0])
                           
        #fine motor control lower right
        if cur_pos[0] < MapLength - 1 and cur_pos[1] < MapLength - 1:
            if game_map[cur_pos[0] + 1][cur_pos[1] + 1]:
                if GameObject == type(game_map[cur_pos[0] + 1][cur_pos[1] + 1][0]):
                    fineLowerRight = self.value(game_map, cur_pos, cur_pos[0] + 1, cur_pos[1] + 1, game_map[cur_pos[0] + 1][cur_pos[1] + 1][0])
           
        #fine motor upper right
        if cur_pos[0] > 0 and cur_pos[1] < MapLength - 1:
            if game_map[cur_pos[0] - 1][cur_pos[1] + 1]:
                if GameObject == type(game_map[cur_pos[0] - 1][cur_pos[1] + 1][0]):
                    fineUpperRight = self.value(game_map, cur_pos, cur_pos[0] - 1, cur_pos[1] + 1, game_map[cur_pos[0] - 1][cur_pos[1] + 1][0])
                           
        #fine motor control lower left
        if cur_pos[0] < MapLength - 1 and cur_pos[1] > 0:
            if game_map[cur_pos[0] + 1][cur_pos[1] - 1]:
                if GameObject == type(game_map[cur_pos[0] + 1][cur_pos[1] - 1][0]):
                    fineLowerLeft = self.value(game_map, cur_pos, cur_pos[0] + 1, cur_pos[1] - 1, game_map[cur_pos[0] + 1][cur_pos[1] - 1][0])
        ...
        if fineMax > 0:
            ...
            assert( direction != 'up' or cur_pos[0] > 0 )
            assert( direction != 'down' or cur_pos[0] < MapLength )
            assert( direction != 'left' or cur_pos[1] > 0 )
            assert( direction != 'right' or cur_pos[1] < MapLength )
                ...
                for j in range(0, MapLength):
                    if game_map[i][j]:
                        if GameObject == type(game_map[i][j][0]):
                            grossUp += self.value(game_map, cur_pos, i, j, game_map[i][j][0])/distance(cur_pos,i,j)

        #gross motor control down
        grossDown = 0
        if cur_pos[0] < MapLength - 1:
            for i in range(cur_pos[0] + 1, MapLength):
                for j in range(0, MapLength):
                    if game_map[i][j]:
                        if GameObject == type(game_map[i][j][0]):
                            grossDown += self.value(game_map, cur_pos, i, j, game_map[i][j][0])/distance(cur_pos,i,j)
        ...
        if cur_pos[1] > 0:
            for i in range(0, MapLength):
                for j in range(0, cur_pos[1]):
                    if game_map[i][j]:
                        if GameObject == type(game_map[i][j][0]):
                            grossLeft += self.value(game_map, cur_pos, i, j, game_map[i][j][0])/distance(cur_pos,i,j)

        #gross motor control right
        grossRight = 0
        if cur_pos[1] < MapLength - 1:
            for i in range(0, MapLength):
                for j in range(cur_pos[1] + 1, MapLength):
                    if game_map[i][j]:
                        if GameObject == type(game_map[i][j][0]):
                            grossRight += self.value(game_map, cur_pos, i, j, game_map[i][j][0])/distance(cur_pos,i,j)
        ...
        #game is over and either we won or lost.  Now just mop up the board
        if grossMax == 0:
        ...
        assert( direction != 'up' or cur_pos[0] > 0 or self.endgame == "true")
        assert( direction != 'down' or cur_pos[0] < MapLength or self.endgame == "true")
        assert( direction != 'left' or cur_pos[1] > 0 or self.endgame == "true" )
        assert( direction != 'right' or cur_pos[1] < MapLength or self.endgame == "true" )
